...
// Title elements (assigned during boot)
let saveTitleEl = null;
...
(async function boot(){
  ...
  saveTitleEl = document.getElementById("save-title");
  if (saveTitleEl){
    saveTitleEl.textContent = CURRENT_SAVE?.name || "Untitled";

    // Enable title editing UI now that elements are available
    setupTitleEditing();
...
editTitleBtn = document.getElementById("edit-save-title");
...
// Wire up title editing (requires DOM to be ready and saveTitleEl/editTitleBtn set)
function setupTitleEditing() {
  if (!editTitleBtn || !saveTitleEl) return;
